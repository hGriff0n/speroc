...
#define pstr(x) pegtl_string_t((x))
			...
			struct sequence : list<T, one<sep>, ig_s> {};
			template<class T, class O, class C, char sep = ','>
			struct col : seq<O, sequence<T, sep>, C> {};								// T ig_s (sep ig_s T ig_s)*
			template<class T, class O, class C, char sep = ','>
			struct opt_col : opt<col<T, O, C, sep>> {};									// (T ig_s (sep ig_s T ig_s)*)?
			...
			struct tuple : seq<oparen, opt<list<seq<ig_s, expr>, one<','>>>, cparen> {};
			struct array : seq<obrack, opt<list<seq<ig_s, expr>, one<','>>>, cbrack> {};
			...
			struct pat_tuple : seq<oparen, list<seq<ig_s, pattern>, one<','>>, one<')'>> {};
			struct pattern : sor<placeholder, var, pat_tuple, seq<typ, opt<pat_tuple>>> {};
			struct case_stmt : seq<list<seq<ig_s, pattern, ig_s>, one<','>>, pstr("->"), ig_s, expr> {};
			...
			struct type_tuple : seq<oparen, list<seq<ig_s, mut_type>, one<','>>, cparen> {};
			...
			struct generic : seq<obrack, list<seq<ig_s, gen_part>, one<','>>, cbrack> {};
			...
			struct constructors : list<seq<ig_s, cons>, one<'|'>> {};
			...
			struct use_path : seq<list<sor<var, seq<obrace, list<variable, one<','>>, cbrace>>, one<':'>>, one<':'>> {};
			struct use_elem : sor<seq<variable, opt<pstr("as"), ig_s, variable>>, seq<typ, ig_s, opt<pstr("as"), ig_s, typ, ig_s>>> {};
			struct mod_use : seq<k_use, opt<use_path>, sor<one<'*'>, opt<obrace, list<use_elem, one<','>>, use_elem>>> {};
