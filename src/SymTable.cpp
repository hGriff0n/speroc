...
namespace spero::compiler::analysis {
	...
	ref<_SymTable::StorageType> _SymTable::operator[](std::string key) {
		return vars[key];
	}
	bool _SymTable::exists(std::string key) const {
		return vars.count(key) != 0;
	}

	// Accessor interfaces
	opt<ref<_SymTable::StorageType>> _SymTable::get(std::string key) {
		if (exists(key)) {
			return operator[](key);
		}
	...
	opt<ref<_SymTable>> _SymTable::getScope(std::string key) {
		if (auto data = get(key)) {
			if (auto* table = std::get_if<ref<_SymTable>>(&data->get())) {
				return table->get();
			}
		}
	...
	opt<ref<SsaVector>> _SymTable::getOverloadSet(std::string key) {
		if (auto data = get(key)) {
	...
	opt<ref<_VarData>> _SymTable::getVariable(std::string key, size_t ssa_id) {
	...
	opt<_SymTable::DataType> _SymTable::ssaIndex(std::string key, opt<size_t>& index, const Location& loc) {
		if (auto data = get(key)) {
			if (auto* vec = std::get_if<SsaVector>(&data->get())) {
				if (!index) {
				...
				return std::get<ref<_SymTable>>(data->get());
			}
		}
	...
	// Mutation interfaces
	bool _SymTable::insert(std::string key, _VarData value) {
		if (!exists(key)) {
			vars[key] = SsaVector{};
		}
	...
	bool _SymTable::insert(std::string key, ref<_SymTable> value) {
		if (!exists(key)) {
			vars[key] = value.get();
			return true;
		}
	...
	void _SymTable::setContextRules(ScopingContext rule) {
		rules = rule;
	}
	ScopingContext _SymTable::getContextRules() const {
		return rules;
	}

	// Counting interfaces
	size_t _SymTable::size() const {
		return vars.size();
	}
	...
	size_t _SymTable::count(std::string key) const {
		return vars.count(key);
	}
