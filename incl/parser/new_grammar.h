...
namespace spero::parser::grammar::next {
    ...
    #define DEF_BINPREC_LEVEL(lvl, prev) \
    ...
    #define DEF_BINPREC_LEVEL_OP(lvl, prev, ...) \
        struct binary_opch##lvl : one_of<__VA_ARGS__> {}; \
        DEF_BINPREC_LEVEL(lvl, prev)
    ...
    */
    ...
    // Error if 'Sep Sep' or no 'Rule': immediate
	template<class Odelim, class Rule, class Cdelim, class Sep = comma>
	struct sequence : seq<Odelim, Rule, star<Sep, Rule>, Cdelim> {};
    struct check_import {
       template<tao::pegtl::apply_mode, tao::pegtl::rewind_mode, class, class, class Input>
       static bool match(Input& in, Stack& s, CompilationState&) {
           // make sure a raw import doesn't import a specialized generic without being rebound
       }
    };
    struct check_alias {
        template<tao::pegtl::apply_mode, tao::pegtl::rewind_mode, class, class, class Input>
        static bool match(Input& in, Stack& s, CompilationState&) {
            // return true if the old name type matches the new name type
        }
     };
    ...
    // Error if 'var' is a keyword (analysis step ??)
    ...
    struct varname : seq<var, opt<_array>> {};
    struct typname : seq<typ, opt<_array>> {};
    struct name : sor<varname, typname> {};
    struct path : star<name, one<':'>> {};
    struct variable : seq<path, varname> {};
    struct fulltype : seq<path, typname> {};
    struct qualop : seq<path, op> {};


    /*
     * Assignment Patterns
     */
    struct pat_tuple : opt_sequence<oparen, pattern, sor<cparen, errorparen>> {};
    // Error if 'type' not used: immediate
    struct pat_adt : seq<fulltype, ig_s, opt<pat_tuple>> {};
    struct capture_desc : sor<seq<one<'&'>, ig_s, opt<kmut>>, kmut, eps> {};
    // TODO: Add action to pop off 'var'
    struct pat_name_bad_path : one<':'> {};
    struct pat_name : seq<var, not_at<pat_name_bad_path>, ig_s> {};
    // Error if invalid capture_desc or pat_tuple/pat_name not used
    struct capture : seq<capture_desc, sor<pat_tuple, pat_name>> {};
    struct pat_any : disable<plambda> {};
    struct pattern : seq<sor<pat_any, capture, pat_lit, pat_adt>, ig_s> {};
    struct assign_tuple : sequence<oparen, assign_pat, sor<cparen, errorparen>> {};
    struct assign_drop : disable<plambda> {};
    struct assign_name : sor<var, op> {};
    // Error if none of the assign patterns matches
    struct assign_pat : seq<sor<assign_name, assign_drop, assign_tuple>, ig_s> {};
    struct assign_type : seq<typ, ig_s> {};
    ...
    struct single_type : seq<typname> {};
    struct ref_type : seq<single_type, opt<ptr_styling, ig_s>> {};
    struct tuple_type : opt_sequence<oparen, type, sor<cparen, errorparen>> {};
    // Error if no type used
	...
	// Error if no type used (deferred)
	...
	// Error if 'typ' not used, at '(' and no tuple_type (deferred)
	 ...
	 * Control Structures
	 */
	// Error if no mvexpr or mvdexpr (deferred)
	...
	// Error if 'elseif' used (immediate)
	// Error if no mvexpr (deferred)
	...
	// Error if no '=>' used (deferred)
	// Error if no valid end character (';}\n') (immediate?)
	struct _case : seq<case_pat, opt<if_core>, pstr("=>"), ig_s, mvexpr, opt<endc>> {};
	// Error if no opening '{' (immediate-deferred)
	...
	// Error if ',' and no mvexpr (immediate)
	...
	// Error if no mvexpr used (deferred)
	struct lambda : seq<pstr("->"), ig_s, mvexpr> {};
	struct fwd_dot : seq<dot> {};
	// Error if no dot_ctrl or index_cont (deferred)
     ...
     * Expressions
     */
	// Error if '=' not used, 'in' not used (immediate)
	// Error if wrong keyword used (deferred?)
    // Error if exprs not used (deferred)
    struct in_assign : seq<vcontext, assign_pat, opt<_generic>, opt<type_inf>, equals, valexpr, kin, mvexpr> {};
    struct call : enable<tuple> {};
    // NOTE: `type_const` and `var_val` have been changed
    struct type_const : seq<typ, ig_s, opt<disable<call>, opt<anon_type>>> {};
	...
	// Error if '.' and no fncall (deferred)
	struct index_cont : seq<indexeps, fncall, star<dot, fncall>> {};
	// Error if '.' and no dot_ctrl or index_cont (deferred)
	struct index : seq<fncall, opt<dot, sor<dot_ctrl, index_cont>>> {};
	// Error if unop and no index (deferred)
     ...
     * Statements
     */
	// Error if eolf or ';' not used (immediate)
	...
	// Error if no scope, no type used (deferred)
    struct impl : seq<kimpl, single_type, opt<for_type>, opt<impl_errchars>, scope> {};
    // NOTE: `mulimp` to `mod_imp` have been changed
    struct mulimp : sequence<obrace, seq<ig_s, binding, ig_s>, sor<cbrace, errorbrace>> {};
    // Error if no mul_imp, or imp_alias (deferred)
    // Error if rebind name is different than imported (immediate)
    struct alias : seq<pstr("as"), ig_s, name, check_alias> {};
    struct rebind : seq<binding, sor<alias, check_import>> {};
    struct mod_imp : seq<kuse, path, sor<mulimp, rebind> {};
	// Error if '=' not used (immediate)
	// Error if scope not used (deferred)
     ...
     */
    // Error if no control or binexpr (deferred)
    struct valexpr : seq<opt<kmut>, sor<control, binexpr>, opt<type_inf>> {};
    // Error if unexpected keyword (immediate)
    // Error if no valexpr or in_assign (deferred)
    ...
    // Error if no statement (deferred)
    struct annotated : seq<star<annotation>, statement, opt<endc>> {};
    // Look at "forward_set" gobbling
    // Look at adding a "catch" rule, for when no expressions match
